...
class HTTPHTMLParser(Parser):
    ...
    try:
        import os
        with open(os.getenv('PYREST_STYLE_SHEET'), 'r') as f:
            read_data = f.read()
    ...
    def from_categories(self, categories):
        ...
        body += "<div id=\"header\"><a href=\"/\">Home</a> <a href=\"/-/\">Query Interface</a></div>"
        body += "<div id=\"content\"><h1>Registered Categories:</h1>"
        ...
        for cat in categories:
            ...
            body += "</table>"
        body += "</div><div id=\"footer\"><small>Generated by <a href=\"http://pyssf.sf.net\">SSF</a> (c) Platform Computing</small></div>"
    ...
    def from_resource(self, resource):
        ...
        body += "<div id=\"content\"><h1>Resource description</h1>"
        ...
        if len(resource.links) > 0 or len(resource.actions) > 0:
        ...
        if len(resource.attributes.keys()) > 0:
        ...
        if len(backends.REGISTERED_BACKENDS) > 0:
            body += "<h2>Create Resources</h2>"
            body += "<form name=\"input\" action=\"\" method=\"post\">"
            body += "<fieldset><legend>Create a new resource</legend>"
            body += "<p><label>Category</label><select name=\"Category\">"
            for item in backends.REGISTERED_BACKENDS.keys():
                body += "<option value='" + item.term + ";scheme=\"" + item.scheme + "\"'>" + item.scheme + item.term + "</option>"
            body += "</select></p>"
            body += "<p><label>CSV Attribute List</label><input type=\"text\" name=\"Attribute\" value=\"\" /></p>"
            body += "<p><input type=\"submit\" value=\"Create\" /></p></fieldset></form>"

        body += "</div>"
        body += "<div id=\"footer\"><small>Generated by <a href=\"http://pyssf.sf.net\">SSF</a> (c) Platform Computing</small></div>"
    ...
    def from_resources(self, resources):
        ...
        body += "<div id=\"content\"><h1>pyREST OCCI Implementation</h1>"
